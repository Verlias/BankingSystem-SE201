...
public class BankTest {
    ...
    void setUp() {
        bank = new Bank();
        Checking = new Checking(7, ACCOUNT_ID_1);
        CertificateOfDeposit = new CertificateOfDeposit(2000, 5.5, ACCOUNT_ID_2);
    }
    ...
    void bank_has_two_account_in_the_bank_when_two_account_is_added() {
        bank.addAccount(Checking.getId(), Checking);
        bank.addAccount(CertificateOfDeposit.getId(), CertificateOfDeposit);
        int actual = bank.getNumberOfAccounts();
    ...
    void retrieve_one_account_from_bank_and_is_correct_account() {
        bank.addAccount(Checking.getId(), Checking);
        Accounts actual = bank.getAccount().get(Checking.getId());
    ...
    void deposit_money_through_id_and_is_correct_account_and_balance() {
        bank.addAccount(Checking.getId(), Checking);
        bank.addDeposit(Checking.getId(), 100);
        double actual = bank.getAccount().get(Checking.getId()).getBalance();
    ...
    void withdraw_money_through_id_and_is_correct_account_and_balance() {
        bank.addAccount(CertificateOfDeposit.getId(), CertificateOfDeposit);
        bank.withdraw(CertificateOfDeposit.getId(), 100);
