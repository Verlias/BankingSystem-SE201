...
public class CommandProcessorTest {
	...
	void deposit_to_checking_account_within_limit() {
		...
		assertEquals(1000.0, actual, 0.01,
				"Balance after maximum allowed deposit to checking account should be 1000.0");
	}
	...
	void deposit_to_cd_account_not_allowed() {
		...
		assertEquals(1000.0, actual, 0.01,
				"Deposits should not be allowed for CD accounts, so the balance should remain at the initial amount.");
	}
	...
	void withdraw_from_checking_account() {
		...
		assertEquals(300.0, actual, 0.01,
				"Balance should be correctly updated after withdrawing $200 from a checking account.");
	}
	...
	void withdraw_more_than_checking_limit() {
		...
		assertEquals(500.0, actual, 0.01,
				"Balance should remain the same after exceeding the withdrawal limit for checking.");
	}
	...
	void withdraw_from_cd_account_before_12_months() {
		...
		double actual = bank.getAccount().get("12345678").getBalance();
		assertEquals(1000.0, actual, 0.01,
				"Balance should remain the same after attempting to withdraw from a CD account before 12 months.");
	}
	...
	void withdraw_from_empty_account() {
		...
		assertEquals(0.0, actual, 0.01,
				"Balance should remain the same after attempting to withdraw from an empty account.");
	}
	...
	void passtime_checking_account_withdrawal_allowed() {
		...
		assertEquals(600.0, actualBalance, 0.01,
				"Checking account withdrawal should reduce balance by the specified amount (up to $400).");
	}
	...
	void passtime_savings_account_withdrawal_allowed() {
		...
		double actualBalance = bank.getAccount().get("12345678").getBalance();
		assertEquals(1000.0, actualBalance, 0.01,
				"Savings account withdrawal should reduce balance by the specified amount (up to $1000).");
	...
	void passtime_deduction_checking() {
		...
		assertEquals(expectedBalance, actualBalance, 0.01,
				"Low balance should result in a $25 deduction in Checking account.");
	}
	...
	void passtime_deduction_savings() {
		...
		double actualBalance = bank.getAccount().get("12345678").getBalance();
		assertEquals(expectedBalance, actualBalance, 0.01,
				"Low balance should result in a $25 deduction in Savings account and apply APR.");
	}
	...
	void transfer_valid_checking_to_checking() {
		...
		assertEquals(expectedBalanceFrom, actualBalanceFrom, 0.01,
				"Source checking account balance should decrease correctly.");
		assertEquals(expectedBalanceTo, actualBalanceTo, 0.01,
				"Destination checking account balance should increase correctly.");
	}
	...
	void transfer_checking_to_savings() {
		...
		assertEquals(expectedBalanceFrom, actualBalanceFrom, 0.01,
				"Source checking account balance should decrease correctly.");
		assertEquals(expectedBalanceTo, actualBalanceTo, 0.01,
				"Destination savings account balance should increase correctly.");
	}
	...
	void transfer_savings_to_checking() {
		...
		assertEquals(expectedBalanceFrom, actualBalanceFrom, 0.01,
				"Source savings account balance should decrease correctly.");
		assertEquals(expectedBalanceTo, actualBalanceTo, 0.01,
				"Destination checking account balance should increase correctly.");
	}
	...
	void transfer_savings_to_savings() {
		...
		commandProcessor.process("transfer 12345678 98765432 500.0"); // Transfer 500 from one savings account to
																		// another
